...
const DetailPage = ({ deleteJob }) => {
  const { job } = useLoaderData();
  const { id } = useParams();
  const navigate = useNavigate();
  const deleteConfirm = (jobId) => {
    ...
    <>
      <section>
        <div className="container m-auto py-6 px-6">
          ...
          >
            <FaArrowLeft className="fas fa-arrow-left mr-2 text-4xl font-bold" />{" "}
...
export const jobLoader = async ({ params }) => {
  try {
    ...
    console.error("获取工作详情时出错:", error);
    throw new Response("加载工作详情失败", { status: 500 });
  } finally {
    console.log("获取工作详情完成");
  }
};
